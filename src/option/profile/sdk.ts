...
import { SharedIniFile } from './sdk-ini-file';
...
import { APP_NAME, VERSION } from '../../constant';
...
export class SDK {
    ...
    constructor(options: SDKOptions) {
        ...
        AWS.config.update({
            customUserAgent: `${APP_NAME}/${VERSION}`
        });
...
class CredentialsCache {
    ...
    private async getCredentials(awsAccountId: string | undefined, mode: Mode): Promise<AWS.Credentials> {
        ...
        throw new Error(`Need to perform AWS calls for account ${awsAccountId}, but no credentials found.`);
    }
}
